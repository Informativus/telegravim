...
std::span<const ConfigOption> ConfigOptions() {
	static const auto result = std::vector<ConfigOption>{
		...
		{ kOptionVimKeymapKeyCopyMessage, u"Показывает метки для копирования сообщений. Выбранное сообщение копируется с учётом ограничений чата; фотографии копируются как изображение после загрузки. В альбоме есть отдельная метка у каждой фотографии и общая All photos для вставки всех фотографий отдельными файлами. В выделенном тексте y имеет свою контекстную роль."_q },
