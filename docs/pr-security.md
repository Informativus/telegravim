...
changed files, 250 introduced commits, 2 MB per changed blob and 8 MB per diff
are accepted. Introduced commit history has a separate 32 MB budget,
including merge patches; it can repeat the same source data more than once.
The complete history remains secret-scanned without truncation or exclusions.
Source export is limited to 20,000 files/150 MB.
